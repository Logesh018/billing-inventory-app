...
export default function InvoiceForm({ onSubmit, onClose, initialValues = {} }) {
  ...
  useEffect(() => {
    if (initialValues && Object.keys(initialValues).length > 0) {
      setFormData({
        documentNo: initialValues.documentNo || "", // Changed from invoiceNo
        documentDate: initialValues.documentDate ? new Date(initialValues.documentDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0], // Changed from invoiceDate
        dueDate: initialValues.dueDate ? new Date(initialValues.dueDate).toISOString().split('T')[0] : new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
  ...
  const handleFormChange = (field, value) => {
    if (field.startsWith('customerDetails.')) { // Changed from 'customer.'
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Validation
      if (!formData.customerDetails.name.trim()) { // Changed from 'customer.name'
        alert("Customer name is required");
        return;
      }

      if (!formData.customerDetails.mobile.trim()) { // Changed from 'customer.mobile'
      ...
      const submitData = {
        documentType: "invoice", // Added documentType field
        documentNo: formData.documentNo, // Changed from invoiceNo
        documentDate: formData.documentDate, // Changed from invoiceDate
        dueDate: formData.dueDate,
        paymentTerms: formData.paymentTerms,
        notes: formData.notes,
        template: formData.template,
        customization: formData.customization,
        customerDetails: formData.customerDetails, // Changed from 'customer'
  ...
  return (
    <FormContainer
      title={initialValues._id ? "Edit Invoice" : "Create Invoice"}
      onClose={onClose}
    ...
    >
      {/* Invoice Details */}
      <FormSection title="Invoice Details" color="blue">
        <FormGrid columns={4}>
          <FormInput
      ...
      <FormSection title="Customer Information" color="green">
        <FormGrid columns={3}>
          <BuyerSearchInput
            label="Customer Name"
            value={formData.customerDetails.name} // Changed from 'customer.name'
            onChange={(value) => handleFormChange('customerDetails.name', value)} // Changed from 'customer.name'
          ...
          <FormInput
            label="Company"
            value={formData.customerDetails.company} // Changed from 'customer.company'
            onChange={(value) => handleFormChange('customerDetails.company', value)} // Changed from 'customer.company'
            placeholder="Company name (optional)"
          />
          <FormInput
            label="GST Number"
            value={formData.customerDetails.gst} // Changed from 'customer.gst'
            onChange={(value) => handleFormChange('customerDetails.gst', value)} // Changed from 'customer.gst'
        ...
        <FormGrid columns={3} className="mt-2">
          <FormInput
            label="Mobile Number"
            value={formData.customerDetails.mobile} // Changed from 'customer.mobile'
            onChange={(value) => handleFormChange('customerDetails.mobile', value)} // Changed from 'customer.mobile'
          ...
          <FormInput
            label="Email"
            type="email"
            value={formData.customerDetails.email} // Changed from 'customer.email'
            onChange={(value) => handleFormChange('customerDetails.email', value)} // Changed from 'customer.email'
            placeholder="Customer email"
          />
          <FormTextarea
            label="Address"
            value={formData.customerDetails.address} // Changed from 'customer.address'
            onChange={(value) => handleFormChange('customerDetails.address', value)} // Changed from 'customer.address'
      ...
      {/* Invoice Items */}
      <FormSection title="Invoice Items" color="purple">
            ...
            <div key={index} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
              ...
              <FormGrid columns={4} gap={3} className="mt-3">
                ...
                <FormSelect
                  ...
                  options={[
                    { value: "percentage", label: "Percentage (%)" }, // Added 'percentage'
      ...
      <FormSection title="Invoice Summary" color="yellow">
      ...
      <FormSection title="Additional Information" color="gray">
        <FormGrid columns={1}>
          <FormTextarea
            ...
            placeholder="Payment terms, notes, or additional information for the invoice"
            rows={3}
          />
        </FormGrid>
      </FormSection>
