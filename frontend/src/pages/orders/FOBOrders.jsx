...
export default function FOBOrders() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const fobOrders = Array.isArray(ordersData)
        ? ordersData.filter(o => o.orderType === "FOB")
        : [];
      setOrders(fobOrders);
      setError(null);
    } catch (error) {
      console.error("Error fetching FOB orders:", error);
      setError("Failed to fetch FOB orders");
      setOrders([]);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-64">
        <div className="text-lg text-gray-600">Loading FOB orders...</div>
      </div>
    );
  }
      ...
      {showForm ? (
        ...
        <>
          <div className="flex justify-between items-center">
            ...
            >
              <Plus className="w-4 h-4 mr-1" /> Create FOB Order
            </button>
          </div>
          ...
          {orders.length > 0 ? (
            ...
            <div className="text-center py-12 bg-gray-50 rounded-lg">
              <div className="text-gray-500 text-lg mb-2">No FOB orders found</div>
              <div className="text-gray-400 text-sm mb-4">
                Create your first FOB order to get started
              </div>
              <button
