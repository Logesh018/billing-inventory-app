...
export class NotePDFGenerator extends BasePDFGenerator {
  ...
  async generate(note, cloudinaryService) {
    try {
      ...
      console.log(`   Source: ${note.isAutoGenerated ? 'Auto-generated' : 'Manual'}`);
      if (note.PURTNo) {
        console.log(`   Purchase Return: ${note.PURTNo}`);
      }
      ...
      // ✅ Determine if this is inter-state or intra-state
      const partyState = note.partyDetails?.state || "";
      const businessState = note.businessDetails?.state || "Tamil Nadu";
      ...
      // Generate HTML sections with GST type awareness
      const itemsHTML = this.generateNoteItemsHTML(note);
      const taxRowsHTML = this.generateNoteTaxRows(note, isInterState);
      const additionalChargesHTML = this.generateAdditionalChargesHTML(note, isInterState);
      ...
      // Add Purchase Return reference if auto-generated
      let reasonDescription = note.reasonDescription || "";
      if (note.isAutoGenerated && note.PURTNo) {
        reasonDescription = `Auto-generated from Purchase Return ${note.PURTNo}. ${reasonDescription}`;
      }
      ...
      const data = {
        ...
        "business.address": note.businessDetails?.address || "31/4, Kamaraj Nagar, Gandhi Nagar, Karamadai",
        "business.city": note.businessDetails?.city || "Mettupalayam",
        "business.state": businessState,
        "business.pincode": note.businessDetails?.pincode || "641104",
        "business.gst": note.businessDetails?.gst || "33AAVFN6955C1ZX",
        "business.pan": note.businessDetails?.pan || "AAVFN6955C",
        "business.accountNumber": note.businessDetails?.accountNumber || "42495390526",
        "business.ifsc": note.businessDetails?.ifsc || "SBIN0001384",
        "business.branch": note.businessDetails?.branch || "SBI Bank, Mettupalayam",
        "business.bankName": note.businessDetails?.bankName || "State Bank of India",
        ...
        "taxes.igst": note.taxes?.igst?.toFixed(2) || "0.00",
      ...
      console.log(`   CGST: ₹${data["taxes.cgst"]} | SGST: ₹${data["taxes.sgst"]} | IGST: ₹${data["taxes.igst"]}`);
   ...
   * @param {Boolean} isInterState - Whether this is inter-state transaction
   */
  generateNoteTaxRows(note, isInterState = false) {
    let html = '';

    if (isInterState) {
      // ✅ IGST for inter-state
      console.log('   Generating IGST tax rows');
      ...
      Object.keys(taxGroups).sort().forEach(rate => {
        ...
        group.igst = (group.taxableAmount * parseFloat(rate)) / 100;

        html += `
        <tr class="full-border">
          <td class="left" colspan="4">Output IGST</td>
          <td class="center">${rate}%</td>
          <td class="right">₹${group.igst.toFixed(2)}</td>
        </tr>
      `;
      });
    } else {
      // ✅ CGST + SGST for intra-state
      console.log('   Generating CGST+SGST tax rows');
   ...
   * @param {Boolean} isInterState - Whether this is inter-state transaction
   */
  generateAdditionalChargesHTML(note, isInterState = false) {
